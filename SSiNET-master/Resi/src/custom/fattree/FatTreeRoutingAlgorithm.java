...
public class FatTreeRoutingAlgorithm implements RoutingAlgorithm, Cloneable {
	...
	private void buildEdgeSwitchesTable(int k,int numEachPod) {
	 ...
	 private void buildAggSwitchesTable(int k,int numEachPod) { 
		 /**this function build a prefĩx-sufix routing table for Aggswitches
		  * 
		  */
	for (int p = 0; p < k; p++) {
           int offset = numEachPod * p;
...
private void buildCoreSwitchesTable(int k, int numEachPod) {
	/**this function build prefix-suffix routing talbe 
	 * 
	 */
    ...
    private void buildTables() {
        ...
        // edge switches
        this.buildEdgeSwitchesTable(k, numEachPod);
      
        // agg switches
       this.buildAggSwitchesTable(k, numEachPod);

        // core switches
        this.buildCoreSwitchesTable(k, numEachPod);

        System.out.println();
    }
    
    private int noname(int current,int destination){
    ...
    public int next(int source, int current, int destination) {
        if (G.isHostVertex(current)) {
            ...
            return this.noname(current, destination);
        }
    }

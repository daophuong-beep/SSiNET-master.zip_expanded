...
public class FatTreeGraph extends Graph {
   ...
   private void severedge(int k, int offset) {
	   for (int e = 0; e < k / 2; e++) {
   ...
   private void aggedge(int k, int offset) {
	   for (int e = 0; e < k / 2; e++) {
           int edgeSwitch = offset + k * k / 4 + e;
           for (int a = k / 2; a < k; a++) {
   ...
   private void aggcore(int k ,int offset) {
	   for (int a = 0; a < k / 2; a++) {
    ...
    private void sevevswitch(int k) {
       ...
       for (int p = 0; p < k; p++) {
           ...
           this.severedge(k,offset);         
         this.aggedge(k, offset);
        this.aggcore(k, offset);
       }
   }
    ...
    this.sevevswitch(k);
    buildPodSwitchesAdr ();
    buildCoreSwitchesAdr(); 
    buildSeversAdr();
    }
    private void buildPodSwitchesAdr ( ) {
    /** this function is building address for pod switches
     * 	the address is given of the form 10.p.s.1 with p is the 
     * pod number and s is the position of switch in the pod  
     */
    ...
    private void buildCoreSwitchesAdr() {
    /**this function is building address for core switches 
     * the address is given of the form 10.k.j.i with j,i is the
     * switch’s coordinates in the (k/2)2 core switch
     * 	  
     */
    	this.address = new Address[V];
    ...
    private void buildSeversAdr() {
    /** this function is building address for server 
     * the address is given of the form 10.p.e.h with e is 
     * position podswitch that server connecting , h is the position
     * of server in the subnet
     * 	 
     */
    	this.address = new Address[V];
         int numEachPod = k * k / 4 + k;
